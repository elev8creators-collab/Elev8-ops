...
import { TEAM, todayStr, fmtDate } from '../lib/config.js'
    ...
    return (
      <div style={{ maxWidth: 400, margin: '2.5rem auto' }}>
        <div style={{ textAlign: 'center', marginBottom: '1.5rem' }}>
          ...
          <div style={{ fontSize: 13, color: T.muted }}>Select your name to log your day</div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 12 }}>
          {TEAM.map(m => (
            <button key={m.id} onClick={() => { setMember(m); setSub('log') }} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  ...
                  fontSize: 11, fontWeight: 700, color: '#E24B4A',
                }}>{m.initials}</div>
        ...
        <div style={{ border: '1.5px dashed rgba(255,255,255,.08)', borderRadius: 12, padding: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              ...
              style={{
                flex: 1, fontSize: 13, padding: '9px 11px', borderRadius: 8,
                border: '1px solid rgba(255,255,255,.06)', background: '#18181F',
            ...
            <Btn
              onClick={() => {
                if (guestName.trim()) {
                  setMember({ id: 'guest', name: guestName.trim(), role: 'editor', initials: guestName.trim().slice(0,2).toUpperCase() })
                  setSub('log')
                }
              }}
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <Btn variant="ghost" onClick={() => { setMember(null); setGuestName('') }} style={{ fontSize: 12, padding: '5px 10px' }}>
            Switch
          </Btn>
        </div>
      </div>
