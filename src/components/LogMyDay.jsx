...
import { getSettings, saveLog, loadDayLogs } from '../lib/supabase.js'
...
function newEntry(taskTypes) {
  return { id: Date.now()+Math.random(), typeId: taskTypes[0]?.id || 'reel', complexity:'standard', client:'', frameio:'', note:'' }
}
...
export default function LogMyDay({ member, today, onSignOut }) {
  ...
  useEffect(() => {
    getSettings().then(s => {
      ...
      // Check if already submitted TODAY — but always show fresh form when logging back in
      // (submitted state is local to this session only)
    })
  }, [])
  ...
  const { taskTypes, complexity: complexityLevels, weeklyTargets } = settings
    ...
    return (
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'60vh' }}>
        <div style={{ textAlign:'center', maxWidth:400 }} className="fade-in">
          ...
          <div style={{ fontSize:13, color:C.text3, marginBottom:'0.5rem' }}>
            {fmtDate(today)}
          </div>

          {/* Credits earned */}
          <div style={{
            margin:'1.5rem auto', padding:'1.25rem 2rem',
            background:C.redDim, border:`1px solid ${C.redBorder}`,
            borderRadius:14, display:'inline-block',
          }}>
            <div style={{ fontSize:48, fontWeight:900, color:C.red, lineHeight:1 }}>{totalCredits}</div>
          ...
          <div style={{ marginBottom:'2rem' }}>
            {entriesWithCredits.map((e,i) => (
              <div key={i} style={{ display:'flex', justifyContent:'space-between', fontSize:13,
                ...
                <span style={{ color:C.text2 }}>{e.typeLabel} · <span style={{ color:C.text3, textTransform:'capitalize' }}>{e.complexity}</span></span>
                <span style={{ fontWeight:700, color:C.red }}>{e.credits} cr</span>
              </div>
            ))}
          </div>
  ...
  return (
    <div style={{ maxWidth:700 }} className="fade-in">
      ...
      {entriesWithCredits.map((e, idx) => {
        ...
        return (
          <Card key={e.id} style={{ marginBottom:12 }}>
            ...
            {/* Header: task number + credit badge */}
            ...
            <div>
              ...
              <textarea
                placeholder="Describe the work: tools used, why it was complex, anything relevant…"
                value={e.note}
      ...
      {/* Add task */}
      <button
        onClick={() => setEntries(p => [...p, newEntry(taskTypes)])}
        style={{
          width:'100%', padding:'11px', fontSize:13, borderRadius:10,
          border:`2px dashed ${C.border2}`, background:'none',
          color:C.text3, cursor:'pointer', marginBottom:'1.5rem', fontFamily:'inherit',
      ...
      {error && (
        <div style={{ fontSize:12, color:C.danger, marginBottom:12, padding:'8px 14px',
